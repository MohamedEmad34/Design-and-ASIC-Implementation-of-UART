...
```
   ...
   TX_P_DATA ───────────►│                              │
   TX_DATA_VALID ────────►│   ┌────────┐      ┌────────┐ │
   PAR_EN / PAR_TYP ─────►│   │ UART_TX│─────►│ UART_RX│ │──► RX_P_DATA
   Prescale ─────────────►│   └────────┘ TX_OUT└────────┘ │──► RX_DATA_VALID
                         │                              │──► PARITY_ERROR
                         │                              │──► FRAMING_ERROR
                         └─────────────────────────────┘
```
...
uart-verilog/
├── rtl/
│   ├── UART_TOP.v          # Top-level: wires TX and RX together
│   ├── UART_TX.v           # TX top module
│   ├── TX_FSM.v            # TX control FSM + baud-rate tick generator
│   ├── serializer.v        # Parallel-to-serial shift register
│   ├── mux.v                # Selects TX_OUT source per FSM state
│   ├── UART_RX.v           # RX top module
│   ├── uart_rx_fsm.v       # RX control FSM (IDLE/START/DATA/PARITY/STOP/ERROR_CHK)
│   ├── edge_bit_counter.v # Baud-rate divider + bit position counter
...
│   ├── parity_calc.v      # TX parity generator
│   └── parity_check.v      # RX parity checker
│   └── stop_check.v        # RX stop-bit / framing checker
├── tb/
│   └── (testbenches for UART_TOP / UART_RX / UART_TX)
├── docs/
│   └── waveforms, block diagrams
└── README.md
```
...
- ✅ Plain data transfer, no parity (multiple byte patterns, including all-zeros and all-ones edge cases)
- ✅ Even parity
- ✅ Odd parity
- ✅ Injected parity error
- ✅ Injected framing error (corrupted stop bit)
- ✅ Combined parity **and** framing errors in the same frame
- ✅ Back-to-back frame reception at different `Prescale` values

All error flags are held (sticky) from the moment they are detected until the next frame starts, so they can be reliably sampled by a testbench or downstream logic regardless of exactly when it checks.
